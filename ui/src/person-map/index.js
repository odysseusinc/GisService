define([
	...
	const TYPE = 'atlas-person-widget';
	const NAME = `${TYPE}-map`;
	...
	if (!customElements.get(NAME)) {
		...
		class PersonMapWidget extends BaseMapWidget {
			static TYPE = TYPE;
			static TITLE = 'Map';
			...
			attributeChangedCallback(name, oldValue, newValue) {
				if (!this.mapInitiated && this.personId && this.sourceKey) {
					this.initMap();
				}
			}
			...
			loadLocationHistory() {
				return httpQuery(config.gisServiceUrl + `/person/${this.personId}/bounds/${this.sourceKey}`);
			}

			async initMap() {
				const locationHistory = await this.loadLocationHistory();
